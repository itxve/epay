...
$grouprow = $DB->getRow("SELECT settle_open,settle_type,settle_rate FROM pre_group WHERE gid='{$userrow['gid']}' LIMIT 1");
if (!$grouprow) $grouprow = $DB->getRow("SELECT settle_open,settle_type,settle_rate FROM pre_group WHERE gid=0 LIMIT 1");
...
if (!isNullOrEmpty($grouprow['settle_rate'])) $conf['settle_rate'] = $grouprow['settle_rate'];
...
if (isset($_GET['act']) && $_GET['act'] == 'do') {
    if ($_POST['submit'] == '申请提现') {
        ...
        $data = ['uid' => $uid, 'auto' => 0, 'type' => $userrow['settle_id'], 'account' => $userrow['account'], 'username' => $userrow['username'], 'money' => $money, 'realmoney' => $realmoney, 'addtime' => 'NOW()', 'status' => 0];
                ...
                <div class="panel-body">
                    <form class="form-horizontal devform" action="./apply.php?act=do" method="post">
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <div class="input-group"><input class="form-control" type="text"
                                                                value="<?php echo display_type($userrow['settle_id']) ?>"
                                                                disabled><a href="./editinfo.php"
                                                                            class="input-group-addon">修改收款账号</a>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input class="form-control" type="text" value="<?php echo $userrow['money'] ?>"
                                       disabled>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input class="form-control" type="text" name="tmoney"
                                       value="<?php echo $enable_money ?>" disabled>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">申请提现余额</label>
                            <div class="col-sm-9">
                                <div class="input-group"><input class="form-control" type="text" name="money" value=""
                                                                required><a href="javascript:inputMoney()"
                                                                            class="input-group-addon">全部</a></div>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-sm-4"><input type="submit" name="submit" value="申请提现"
                    ...
                    <footer class="panel-footer">
                        <div class="col-sm-offset-2 col-sm-6"><br/>
                            ...
                            当前手动提现模式是：<?php echo $conf['settle_type'] == 1 ? '<b>T+1</b>，可提现余额为截止到前一天你的收入' : '<b>T+0</b>，可提现余额为截止到现在你的收入'; ?>
                            <br/>
