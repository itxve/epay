...
switch ($act) {
    ...
    case 'saveChannel':
        if ($_POST['action'] == 'add') {
            ...
            $data = ['name' => $name, 'rate' => $rate, 'mode' => $mode, 'type' => $type, 'plugin' => $plugin, 'daytop' => $daytop, 'paymin' => $paymin, 'paymax' => $paymax];
            ...
            if ($nrow)
                exit('{"code":-1,"msg":"支付通道名称重复"}');
            $data = ['name' => $name, 'rate' => $rate, 'mode' => $mode, 'type' => $type, 'plugin' => $plugin, 'daytop' => $daytop, 'paymin' => $paymin, 'paymax' => $paymax, 'appid' => $row['appid'], 'appkey' => $row['appkey'], 'appsecret' => $row['appsecret'], 'appurl' => $row['appurl'], 'appmchid' => $row['appmchid'], 'apptype' => $row['apptype'], 'appwxmp' => $row['appwxmp'], 'appwxa' => $row['appwxa'], 'appswitch' => $row['appswitch']];
