...
case 'set_receiver':
	...
	if($status == 1){
		$result = \lib\ProfitSharing::addReceiver($channel, $row['account'], $row['name']);
	}elseif($status == 0){
		$result = \lib\ProfitSharing::deleteReceiver($channel, $row['account'], $row['name']);
	}
	if($result['code'] == 0){
...
case 'del_receiver':
	...
	if($row['status']==1){
		...
		$channel = \lib\Channel::get($row['channel'], $channelinfo);
		if($channel){
			\lib\ProfitSharing::deleteReceiver($channel, $row['account'], $row['name']);
		}
	}
...
case 'add_receiver2':
	...
	if($id = $DB->insert('psreceiver2', $data)){
		$result = \lib\ProfitSharing::addReceiver_adapay($channel, $id, $data);
		if($result['code'] != 0){
			$DB->rollBack();
			exit(json_encode($result));
		}
...
case 'edit_receiver2':
	...
	if($data['card_id']!=$row['card_id'] || $data['card_name']!=$row['card_name'] || $data['cert_id']!=$row['cert_id']){
		$result = \lib\ProfitSharing::editReceiver_adapay($channel, $row['id'], $data, $row['settleid']);
		if($result['code'] != 0){
			exit(json_encode($result));
		}
...
case 'del_receiver2':
	...
	if($channel){
		\lib\ProfitSharing::deleteReceiver_adapay($channel, $row['id'], $row['settleid']);
	}
...
case 'unfreeeze':
	...
	$channel = \lib\Channel::get($row['channel'], $channelinfo);
	if(!$channel) exit('{"code":-1,"msg":"通道信息不存在"}');
	$result = \lib\ProfitSharing::unfreeeze($channel, $row['trade_no'], $row['api_trade_no']);
	if($result['code'] == 0){
...
case 'return':
	...
	$channel = \lib\Channel::get($row['channel'], $channelinfo);
	if(!$channel) exit('{"code":-1,"msg":"通道信息不存在"}');
	$result = \lib\ProfitSharing::return($channel, $row['trade_no'], $row['api_trade_no'], $row['account'], $row['money']);
	if($result['code'] == 0){
		$DB->update('psorder', ['status'=>4], ['id'=>$id]);
	}
	exit(json_encode($result));
