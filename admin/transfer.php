...
if(isset($_POST['submit'])){
	...
	if($app=='alipay' || $app=='bank'){
		$channelid = isset($_POST['channel'])?$_POST['channel']:$conf['transfer_alipay'];
...
<?php if($app=='alipay'){
	$alipay_channel = $DB->getAll("SELECT * FROM pre_channel WHERE plugin='alipay' OR plugin='jeepay'");
	?>
          <form action="?app=alipay" method="POST" role="form">
		    <div class="form-group">
				...
				<select name="channel" class="form-control" default="<?php echo $conf['transfer_alipay']?>">
					<?php foreach($alipay_channel as $channel){echo '<option value="'.$channel['id'].'">'.$channel['name'].''.($channel['id']==$conf['transfer_alipay']?'（默认）':'').'</option>';} ?>
				</select>
			</div></div>
			<div class="form-group">
		  ...
		  <font color="green">支付宝UID：即为mapi密钥查看页面的PID，2088开头的16位数字，或<a href="./gettoken.php?app=alipayuid">点此获取</a></font>
<?php }elseif($app=='wxpay'){?>
		  ...
		  <font color="green">Openid获取地址，<a href="./gettoken.php?app=wechat">点此获取</a></font>
...
function alipayQuery(){
	layer.prompt({title: '填写当前支付宝的UID或openid', value: $.cookie('alipay_user_id'), formType: 0}, function(text, index){
