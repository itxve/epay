...
//            'money' => trim($_POST['money']),
    ...
    case 'user_settle_info':
        ...
        $data = '<div class="form-group"><div class="input-group"><div class="input-group-addon">结算方式</div><select class="form-control" id="pay_type" default="' . $rows['settle_id'] . '">' . ($conf['settle_alipay'] ? '<option value="1">支付宝</option>' : null) . '' . ($conf['settle_wxpay'] ? '<option value="2">微信</option>' : null) . '' . ($conf['settle_qqpay'] ? '<option value="3">QQ钱包</option>' : null) . '' . ($conf['settle_bank'] ? '<option value="4">银行卡</option>' : null) . '' . ($conf['settle_usdt'] ? '<option value="5">USDT-trc20</option>' : null) . '' . ($conf['settle_bnusdt'] ? '<option value="6">币安USDT-trc20</option>' : null) . '</select></div></div>';
