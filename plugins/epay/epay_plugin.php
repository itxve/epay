...
class epay_plugin
{
    static public $info = [
        ...
        'author' => '彩虹', //支付插件作者
        'link' => '', //支付插件作者链接
        'types' => ['alipay', 'qqpay', 'wxpay', 'bank', 'jdpay', 'usdt'], //支付插件支持的支付方式，可选的有alipay,qqpay,wxpay,bank
    ...
    {
        try {
            list($method, $url) = self::pay_mapi('usdt');
        } catch (Exception $ex) {
            return ['type' => 'error', 'msg' => $ex->getMessage()];
        }
        ...
        if ($method == 'jump') {
            ...
            return ['type' => 'qrcode', 'page' => 'usdt_qrcode', 'url' => $url];
        }
    }
    ...
    static public function refund($order)
    {
        ...
        $epay = new EpayCore($epay_config);
        $result = $epay->refund($order['api_trade_no'], $order['refundmoney']);
