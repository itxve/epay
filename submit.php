...
if($conf['pay_iplimit'] > 0){
	...
	$ipcount = $DB->getColumn("select count(*) from pre_order where `ip`='$clientip' and addtime>'$thtime' and status>0");
...
if($firstGetChannel){
	...
	$DB->exec("UPDATE pre_order SET type='{$submitData['typeid']}',channel='{$submitData['channel']}',subchannel='{$submitData['subchannel']}',realmoney='$realmoney',getmoney='$getmoney' WHERE trade_no='$trade_no'");
}
